...
function validarFormulario(nombre, servicio, fecha, hora) {
  let ok = true;
  if (!nombre.trim())  { mostrarError("nombre");   ok = false; }
  if (!servicio)       { mostrarError("servicio");  ok = false; }
  if (!fecha) {
    mostrarError("fecha"); ok = false;
  } else {
    const hoy = new Date(); hoy.setHours(0,0,0,0);
    if (new Date(fecha + "T00:00:00") < hoy) { mostrarError("fecha"); ok = false; }
  }
  if (!hora) { mostrarError("hora"); ok = false; }
  return ok;
}
...
function generarTicket() {
  ...
  if (!validarFormulario(nombre, servicio, fecha, hora)) return;
  ...
  document.getElementById("tk-fecha").textContent    = formatearFecha(new Date(fecha + "T00:00:00"));
  document.getElementById("tk-hora").textContent     = hora;
  document.getElementById("tk-vence").textContent    = calcularVencimiento(fecha, hora);
...
function nuevoTicket() {
  ["nombre","servicio","fecha","hora"].forEach(id => document.getElementById(id).value = "");
